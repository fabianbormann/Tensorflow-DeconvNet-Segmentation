...
image_path='/scratch/gallowaa/birds/images/'
segmentation_path='/scratch/gallowaa/birds/segmentations/'
try:
    os.stat(path)
except:
    pass
...
def read_filelist(image_path, segmentation_path):
    ...
    # This is not a mistake, this dataset happens to use jpeg images and png segmentations
    ...
    filename_queue_segmentations = tf.train.string_input_producer( \
        tf.train.match_filenames_once(segmentation_path+"*/*.png"))
...
s_img = tf.image.decode_png(seg_value) #reads images into tensor
...
with tf.Session() as sess:
    filename = os.path.join('input_data_birds_crop.tfrecords')
